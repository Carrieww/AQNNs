    ...
    for cost in [200, 400, 600, 800]:
        for Pt in [0.95]:
            ...
            PT_precision, PT_recall, PT_k_star, PT_ans = test_PQE_PT(
                Oracle_dist, Proxy_dist, bd=cost, t=Dist_t, prob=Prob, pt=Pt
            )
            PT_end = time.time()
            ...
            for fac in fac_list:
                ...
                rt_prop[fac].append(len(RT_ans) / Oracle_dist.shape[0])
                ...
                pt_prop[fac].append(len(PT_ans) / Oracle_dist.shape[0])

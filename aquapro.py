...
from SRW import RandomWalkSampler_basic
...
def exp_PQA_maximal_CR(
    ...
    # perturbation scale list
    # scale_list = np.array([-20, -15, -10, -5, 0, 5, 10, 15, 20])
    scale_list = np.array([0])
    oracle_ori = oracle
    proxy_ori = proxy

    # we only concern when len(index) = 1, one query only
    for i in range(len(index)):
        query_time_start = time.time()
        print(f"\n>>>>>>>i is {i}")
        # get proxy distance w.r.t. query. Here query is oracle[[index[i]]]
        proxy_dist, _ = preprocess_dist(oracle_ori, proxy_ori, oracle_ori[[index[i]]])
        ...
        prop_default = (len(true_ans) / oracle_ori.shape[0]) * factor
        print(f"the ground truth proportion is {(len(true_ans) / oracle_ori.shape[0])}")
        print(f"c is {prop_default}")
        _, _, GT = one_proportion_z_test(
            len(true_ans), oracle_ori.shape[0], prop_default, 0.05, op
        )
        ...
        if sampling_method == "RNS":
            ...
            SRW_initialization_time = time.time()
            model = RandomWalkSampler_basic(oracle_ori, proxy_ori, t, s, 0)
            print(
                f"finish SRW initialization, using time {round(time.time() - SRW_initialization_time, 2)}"
            )
        # elif sampling_method == "SRW_new":
        #     model = RandomWalkSampler(oracle_ori, proxy_ori, t, prob, pt, s, 0)

        for sample_ind in range(num_sample):
            if oracle_ori.shape[0] < s:
                # logger.info("Sample size cannot be larger than the dataset size.")
                raise ValueError("Sample size cannot be larger than the dataset size.")

            one_sample_start = time.time()

            if sampling_method == "RNS":
                random_indices = np.random.choice(oracle_ori.shape[0], s, replace=False)
                S_oracle = oracle_ori[random_indices]
                S_proxy = proxy_ori[random_indices]

            elif sampling_method == "SRW":
                S_oracle, S_proxy = model.sample()
            # elif sampling_method == "SRW_new":
            #     S_oracle, S_proxy = model.sample()

            proxy_dist, _ = preprocess_dist(S_oracle, S_proxy, oracle_ori[[index[i]]])
            topk, phi = preprocess_topk_phi(proxy_dist, norm_scale=norm_scale, t=t)
            sync_oracle = preprocess_sync(proxy_dist, norm_scale)
            true_ans = np.where(sync_oracle <= t)[0]
    ...
    backup_res = [
        ...
        s,
        # scale_list[0],
        round(sum(pt_k_recall[0]) / len(pt_k_recall[0]), 2),
        round(sum(pt_k_success[0]) / len(pt_k_success[0]), 2),
        round(sum(pt_k_acc[0]) / len(pt_k_acc[0]), 2),
        round(sum(pt_k_rejH0[0]) / len(pt_k_rejH0[0]), 2),
        round(sum(pt_response_time[0]) / len(pt_response_time[0]), 2),
        round(sum(rt_k_precis[0]) / len(rt_k_precis[0]), 2),
        round(sum(rt_k_success[0]) / len(rt_k_success[0]), 2),
        round(sum(rt_k_acc[0]) / len(rt_k_acc[0]), 2),
        round(sum(rt_k_rejH0[0]) / len(rt_k_rejH0[0]), 2),
        round(sum(rt_response_time[0]) / len(rt_response_time[0]), 2),
        prob,
    ]
    if not is_precompile:
        Path("./results/PQA_varying_query/").mkdir(parents=True, exist_ok=True)
        with open(
            "results/PQA_varying_query/"
            + fname
            + "_"
            + sampling_method
            + "_"
            + op
            + f"_{str(pt)[2:]}_0530.txt",
            "a",
        ) as file:
...
def HT_acc(name, ind, ans, total, op, GT, prop_default=None):
    print(f"finished {name} algorithm for index {ind}")
    ...
    print("Accept Null Hypothesis:", reject)

    align = reject == GT
    print("align:", align)

    # z_stat_, p_value_, reject_ = one_proportion_z_test(
    #     true_pos, total, len(true_ans) / total, 0.05, "two-sided"
    # )
    #
    # print("D' Z-Statistic:", z_stat)
    # print("D' P-Value:", p_value)
    # print("D' Accept Null Hypothesis:", accept)
    return align, reject


# TODO: update HT_acc
def exp_CSC_minimal_cost(
    oracle,
    proxy,
    index,
    pt=0.9,
    ...
    for i in range(len(index)):
        ...
        proxy_dist, oracle_dist = preprocess_dist(
            oracle_ori, proxy_ori, oracle_ori[[index[i]]]
        )
        ...
        for sample_ind in range(num_sample):
            ...
            print(f"the ground truth accept result is : {GT[0]}")
            if method == "naive":
                pass
            elif method == "less_naive":
                # sample oracle and proxy
                # prop_default = len(true_ans) / oracle_ori.shape[0]
                random_indices = np.random.choice(
                    oracle_ori.shape[0], num_rows_to_sample, replace=False
                )
                if index[i] in random_indices:
                    pass
                else:
                    random_indices[-1] = index[i]
                assert random_indices.shape[0] == num_rows_to_sample

                oracle = oracle_ori[random_indices]
                proxy = proxy_ori[random_indices]

                proxy_dist, oracle_dist = preprocess_dist(
                    oracle, proxy, oracle_ori[[index[i]]]
                )
                ranks = preprocess_ranks(proxy_dist)
                true_ans = np.where(oracle_dist <= t)[0]
                # print(f"||||||||||||||||||||| num of true ans is {len(true_ans)}")

            K_rt = floor(len(true_ans) * (1 - rt)) + 1
            K_pt = find_K_pt(ranks, oracle_dist, t=t, pt=pt)
            # print(f"k_pt and k_rt are {K_pt}, {K_rt}")
            ...
            def random_start(m, p, o_dist, r, K, s_hat, m_hat, query_type):
                ...
                ans = set_diff(
                    array_union(r[: k_star + 1], r[samples]), r[samples_false]
                )

                true_pos = len(np.intersect1d(ans, true_ans))
                rec = true_pos / len(true_ans)
                prc = true_pos / len(ans)
                # print(f"||||||||||||||||||||| num of true pos is {true_pos}")

                if query_type == "RT":
                    return int(rec >= rt), len(samples), true_pos, true_ans
                elif query_type == "PT":
                    return int(prc >= pt), len(samples), true_pos, true_ans
            ...
            # CSC-RT
            _, _, best_s, best_m, cost = test_CSC_RT(
                oracle_dist, ranks, t=t, prob=prob, rt=rt, mode="exact", K=K_rt
            )

            for _ in range(repeat_new):
                for mode in mode_list:
                    success, cost, true_pos, true_ans = random_start(
                        mode,
                        prob,
                        oracle_dist,
                        ranks,
            ...
            if K_pt == 0:
                for _ in range(repeat_new):
                    precision, _, _, _, cost, _, _, _ = test_CSC_PT(
                        oracle_dist, ranks, t=t, prob=prob, pt=pt, mode="exact", K=0
                    )
                    pt_success["K0"].append(int(precision >= pt))
                    pt_cost["K0"].append(cost)
            else:
                _, _, best_s, best_m, cost, _, _, _ = test_CSC_PT(
                    oracle_dist, ranks, t=t, prob=prob, pt=pt, mode="exact", K=K_pt
                )
                # print(oracle_dist.shape)
                # print(ranks.shape)
                # print(K_pt)
                # print(f"best s and best m are {best_s}, {best_m}")
                for _ in range(repeat_new):
                    for mode in mode_list:
                        success, cost, true_pos, true_ans = random_start(
                            mode,
                        ...
                        if method == "naive":
                            pt_k_acc[mode].append(
                                HT_acc(
                                    "CSC-PT", _, true_pos, true_ans, oracle_ori.shape[0]
                                )
                            )
                        elif method == "less_naive":
                            pt_k_acc[mode].append(
        ...
        print("================================")
        print("time for one query:", round(time.time() - start_time_query, 2))
...
def exp_CSC_cpu_overhead(
    oracle, proxy, index, pt=0.9, rt=0.9, t=0.9, prob=0.9, fname=""
):
        ...
        for _ in range(repeat):
            for mode in mode_list:
                ...
                if K_pt > 0:
                    start_time = time.time()
                    _, _ = find_best_sm(
                        D=len(oracle_dist), K=K_pt, mode=mode, prob=prob
                    )
                    end_time = time.time()
...
def exp_comprehensive_comparison(
    ...
    for i in range(len(index)):
        ...
        for _ in range(repeat):
            ...
            precision, recall, PT_cost = test_CSE_PT(
                oracle_dist, ranks, t=t, prob=prob, pt=pt, mode="approx_m1"
            )
        ...
        for scale in scale_list:
            ...
            for _ in range(repeat):
                ...
                try:
                    precision, recall, _, _ = SUPG(
                        oracle_dist, proxy_dist, t, pt, prob, cost=cost, query_type="PT"
                    )
                ...
                precision, recall, _ = test_PQE_PT(
                    oracle_dist, proxy_dist, bd=cost, t=t, prob=prob, pt=pt
                )
        ...
        for _ in range(repeat):
            ...
            precision, recall, RT_cost = test_CSE_RT(
                oracle_dist,
                ranks,
                t=t,
                delta=delta,
        ...
        for scale in scale_list:
            ...
            try:
                precision, recall, _, _ = SUPG(
                    oracle_dist, proxy_dist, t, rt, prob, cost=cost, query_type="RT"
                )
            except ZeroDivisionError:
                precision = recall = 0
            ...
            precision, recall, _ = test_PQE_RT(
                oracle_dist, proxy_dist, bd=cost, t=t, prob=prob, rt=rt
            )
...
def exp_runningtime(
    oracle, proxy, index, is_sample2test, pt=0.9, rt=0.9, t=0.9, prob=0.9, fname=""
):
    delta = 1 - sqrt(prob)
    rt_prob = prob / (1 - delta)
    CSE_PT_stats = defaultdict(list)
        ...
        while L < R:
            ...
            for _ in range(repeat):
                ...
                if method == "SUPG" and q_type == "PT":
                    ...
                    prec, recl, _ = test_PQE_PT(
                        oracle_dist, proxy_dist, bd=cost, t=t, prob=prob, pt=pt
                    )
                elif method == "PQE" and q_type == "RT":
                    prec, recl, _ = test_PQE_RT(
                        oracle_dist, proxy_dist, bd=cost, t=t, prob=prob, rt=rt
                    )
                elif method == "sample2test" and q_type == "PT":
                    sample2test_ranks = preprocess_ranks(proxy_dist)
                    prec, recl, _ = test_sample2test_PT(
                        oracle_dist, sample2test_ranks, bd=cost, t=t, pt=pt
                    )
                elif method == "sample2test" and q_type == "RT":
                    sample2test_ranks = preprocess_ranks(proxy_dist)
                    prec, recl, _ = test_sample2test_RT(
                        oracle_dist, sample2test_ranks, bd=cost, t=t, rt=rt
                    )
                else:
                    prec = recl = -1
    ...
    for i in range(len(index)):
        ...
        if not is_sample2test:
            ...
            res = binary_search(
                method="sample2test", q_type="PT", cr_target=avg_CSE_PT_CR
            )
    ...
    if is_sample2test:
        ...
        pickle.dump(
            backup_res, open("results/runningtime_sample2test/" + fname + ".pkl", "wb")
        )
    else:
        backup_res = [
            price_l,
...
def exp_scalability_test(
    is_sample2test, pt=0.9, rt=0.9, t=0.9, prob=0.9, fname="", n_query=50
):
    ...
    for subset_size in subset_sizes:
        ...
        for i in range(len(index)):
            proxy_dist, oracle_dist = preprocess_dist(
                subset_oracle, subset_proxy, subset_oracle[[index[i]]]
            )
            ranks = preprocess_ranks(proxy_dist)
            ...
            for _ in range(repeat):
                ...
                if is_sample2test:
                    ...
                    precision, recall, _ = test_sample2test_PT(
                        oracle_dist, ranks, bd=PT_cost, t=t, pt=pt
                    )
                    sample2test_PT_end = time.time()
                    sample2test_PT_stats[subset_size]["success"].append(
                        int(precision >= pt)
                    )
                    sample2test_PT_stats[subset_size]["CR"].append(recall)
                    sample2test_PT_stats[subset_size]["cost"].append(PT_cost)
                    sample2test_PT_stats[subset_size]["overhead"].append(
                        sample2test_PT_end - sample2test_PT_start
                    )
                else:
                    # SUPG-PT
                    SUPG_PT_start = time.time()
                    ...
                    SUPG_PT_stats[subset_size]["overhead"].append(
                        SUPG_PT_end - SUPG_PT_start
                    )

                    # PQE-PT
                    PQE_PT_start = time.time()
                    precision, recall, _ = test_PQE_PT(
                        oracle_dist, proxy_dist, bd=PT_cost, t=t, prob=prob, pt=pt
                    )
                    PQE_PT_end = time.time()
                    ...
                    PQE_PT_stats[subset_size]["overhead"].append(
                        PQE_PT_end - PQE_PT_start
                    )
                ...
                if is_sample2test:
                    ...
                    precision, recall, _ = test_sample2test_RT(
                        oracle_dist, ranks, bd=RT_cost, t=t, rt=rt
                    )
                    sample2test_RT_end = time.time()
                    sample2test_RT_stats[subset_size]["success"].append(
                        int(recall >= rt)
                    )
                    sample2test_RT_stats[subset_size]["CR"].append(precision)
                    sample2test_RT_stats[subset_size]["cost"].append(RT_cost)
                    sample2test_RT_stats[subset_size]["overhead"].append(
                        sample2test_RT_end - sample2test_RT_start
                    )
                else:
                    # SUPG-RT
                    SUPG_RT_start = time.time()
                    ...
                    SUPG_RT_stats[subset_size]["overhead"].append(
                        SUPG_RT_end - SUPG_RT_start
                    )

                    # PQE-RT
                    PQE_RT_start = time.time()
                    precision, recall, _ = test_PQE_RT(
                        oracle_dist, proxy_dist, bd=RT_cost, t=t, prob=prob, rt=rt
                    )
                    PQE_RT_end = time.time()
                    ...
                    PQE_RT_stats[subset_size]["overhead"].append(
                        PQE_RT_end - PQE_RT_start
                    )
    ...
    if is_sample2test:
        ...
        pickle.dump(
            backup_res, open("results/scalability_sample2test/" + fname + ".pkl", "wb")
        )
    else:
        backup_res = [
            subset_sizes,
...
def exp_compare_PQE_PQA(
    oracle, proxy, index, pt=0.9, rt=0.9, t=0.9, prob=0.9, fname="", is_precompile=False
):
    rt_k_success = defaultdict(list)
    rt_k_precis = defaultdict(list)
    pt_k_success = defaultdict(list)
    ...
    for i in range(len(index)):
        ...
        for _ in range(repeat):
            for scale in scale_list:
                ...
                precision, recall, _, _, _ = test_CSC_RT(
                    oracle_dist, ranks, t=t, prob=prob, rt=rt, mode="exact", K=K_scaled
                )
        ...
        for _ in range(repeat):
            for scale in scale_list:
                ...
                precision, recall, _, _, _, _, _, _ = test_CSC_PT(
                    oracle_dist, ranks, t=t, prob=prob, pt=pt, mode="exact", K=K_scaled
                )
...
def exp_compare_sample2test_CSE(
    oracle, proxy, index, pt=0.9, rt=0.9, t=0.9, prob=0.9, fname=""
):
    delta = 1 - sqrt(prob)
    rt_prob = prob / (1 - delta)
    ...
    for i in range(len(index)):
        ...
        for scale in scale_list:
            ...
            for _ in range(repeat):
                ...
                sample2test_PT_start = time.time()
                precision, recall, _ = test_sample2test_PT(
                    oracle_dist, ranks, bd=cost, t=t, pt=pt
                )
                sample2test_PT_end = time.time()
                ...
                sample2test_PT[scale]["overhead"].append(
                    sample2test_PT_end - sample2test_PT_start
                )
        ...
        for scale in scale_list:
            ...
            for _ in range(repeat):
                ...
                sample2test_RT_start = time.time()
                precision, recall, _ = test_sample2test_RT(
                    oracle_dist, ranks, bd=cost, t=t, rt=rt
                )
                sample2test_RT_end = time.time()
                ...
                sample2test_RT[scale]["overhead"].append(
                    sample2test_RT_end - sample2test_RT_start
                )
    ...
    pickle.dump(
        backup_res, open("results/COMP_sample2test_CSE/" + fname + ".pkl", "wb")
    )


def exp_compare_topk_PQA(
    oracle, proxy, index, pt=0.9, rt=0.9, t=0.9, prob=0.9, fname=""
):
    rt_k_success = defaultdict(list)
    rt_k_precis = defaultdict(list)
    ...
    for i in range(len(index)):
        ...
        for j in scale_list:
            ...
            precision, recall, cost = test_topk(
                oracle_dist=sync_oracle, proxy_dist=proxy_dist, scale=j, t=t, prob=prob
            )
...
if __name__ == "__main__":
    ...
    fac_list = np.arange(0.1, 2.0, 0.1)
    fac_list = [round(num, 1) for num in fac_list]

    if Fname == "icd9_eICU":
        # sample_size_list = [8000,8100,8200,8236]
        sample_size_list = list(range(1000, 8001, 1000))
    ...
    # start_time = time.time()
    # num_query = 50
    # for Fname in ["icd9_mimic"]:
    #     Proxy, Oracle = load_data(name=Fname)
    #     np.random.seed(0)
    #     Index = np.random.choice(range(len(Oracle)), size=num_query, replace=False)
    #     if Fname in ["icd9_mimic"]:
    #         exp_CSC_minimal_cost(
    #             Oracle,
    #             Proxy,
    #             Index,
    ...
    # less naive
    # for n_sample in [500, 1500, 2500, 3500]:  # 5000,6000,7000,8000,8236,
    #     for Fname in ["icd9_mimic"]:
    #         Proxy, Oracle = load_data(name=Fname)
    #         np.random.seed(0)
    #         Index = np.random.choice(range(len(Oracle)), size=num_query, replace=False)
    ...
    # for Fname in [
    #     "icd9_mimic"
    # ]:  # 'voc', 'icd9_eICU', 'icd9_mimic', 'jackson10000.csv', 'coco'
    # Proxy, Oracle = load_data(name=Fname)

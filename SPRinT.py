...
if __name__ == "__main__":
    ...
    if args.agg == "pct":
        ...
        if args.Fname in ["eICU", "MIMIC-III"]:
            ...
            args.ori_D_attr = [["", "", item] for item in args.ori_D_attr]
        elif args.Fname in ["Jigsaw"]:
            attr_filename = f"data/{args.Fname}/{args.Fname}_attribute.csv"
            df = pd.read_csv(attr_filename)
            args.ori_D_attr = np.vstack(np.array(df["no_downvote"])).tolist()
            args.ori_D_attr = [["", "", item] for item in args.ori_D_attr]
        else:
    ...
    if args.agg == "pct":
        Proxy_emb, Oracle_emb, _ = create_semi_synthetic_data(Oracle_emb, Proxy_emb, [], factor=args.scalability_factor,
                                                              noise_level=0.001)
    elif args.agg in ["avg", "var", "sum"]:
        Proxy_emb, Oracle_emb, args.ori_D_attr = create_semi_synthetic_data(Oracle_emb, Proxy_emb, args.ori_D_attr,
                                                                            factor=args.scalability_factor,
                                                                            noise_level=0.1)
    else:
        raise ValueError("Unknown aggregation function specified in args.agg")
    ...
    for seed in range(1, 11):
        ...
        if args.agg == "pct":
            ...
            if args.Fname in ["eICU"]:
                ...
                distribution = np.clip(distribution, 0, 500)

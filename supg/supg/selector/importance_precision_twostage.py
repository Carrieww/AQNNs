...
from supg.supg.datasource import DataSource
from supg.supg.sampler import Sampler, ImportanceSampler, SamplingBounds
from supg.supg.selector.base_selector import BaseSelector, ApproxQuery
...
class ImportancePrecisionTwoStageSelector(BaseSelector):
    ...
    def select(self) -> Sequence:
        ...
        cutoff_ub = min(int(math.ceil(tpr_ub * n / self.query.min_precision)), n)
        ...
        return np.unique(np.concatenate([set_inds, samp_inds])), is_bypass

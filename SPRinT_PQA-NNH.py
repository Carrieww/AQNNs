...
def PQE_better(
    ...
    for i in range(args.num_sample):
        ...
        if args.PQA == "PQA":  # PQA no randomness
            indices = np.random.choice(
                Oracle_dist.shape[0], args.total_cost, replace=False
            )
            ...
            if args.method == "P-NNH":
                ...
                # )
                print(
                    f"The number of NN in S is {len(args.true_ans_S)} ({len(args.true_ans_S)/proxy_dist_S.shape[0]}%), the aggregation of true NN is {args.agg_S}"
                )
            ...
            available_indices = np.setdiff1d(
                np.arange(len(oracle_dist_S)), args.fix_sample
            )
            args.available_oracle_dist_S = oracle_dist_S[available_indices]
            args.available_proxy_dist_S = proxy_dist_S[available_indices]

            args.rt = find_optimal_rt(
                args, oracle_dist_S, proxy_dist_S, args.initial_cost
            )
            print(f"find optimal rt {args.rt}")
...
def process_results_NNH(
    ...
    # Saving result
    file_name = f"results/SPRinT_{args.PQA}/{args.method}_{args.Fname}_{args.H1_op}_{args.version}.txt"
    with open(file_name, "a") as file:
        # Write the header (if it's not already present in the file)
        if seed == 1:
            file.write(
                "seed\toptimal cost\tavg acc\tavg recall\tavg precision\tavg f1\tavg fix recall\tavg fix precision\tavg fix f1\tagg ours\tvar ours\tNN ours\tagg_S\tvar agg_S\tNN S\tavg CI\n"
            )

        # Write the data for the current seed
        if avg_CI is None:  # P-NNH
            file.write(
                f"{seed:.4f}\t{args.optimal_cost:.4f}\t{avg_acc:.4f}\t{avg_recall:.4f}\t{avg_precision:.4f}\t{avg_f1}\t{avg_fix_rec:.4f}\t{avg_fix_prec:.4f}\t{avg_fix_f1}\t{avg_agg:.4f}\t{var_agg:.4f}\t{avg_NN_RT:.4f}\t{avg_agg_S}\t{var_agg_S}\t{avg_NN_S:.4f}\t{None}\n"
            )
        else:
            file.write(
                f"{seed:.4f}\t{args.optimal_cost:.4f}\t{avg_acc:.4f}\t{avg_recall:.4f}\t{avg_precision:.4f}\t{avg_f1:.4f}\t{avg_fix_rec:.4f}\t{avg_fix_prec:.4f}\t{avg_fix_f1:.4f}\t{avg_agg:.4f}\t{var_agg:.4f}\t{avg_NN_RT:.4f}\t{avg_agg_S:.4f}\t{var_agg_S:.4f}\t{avg_NN_S:.4f}\t{avg_CI:.4f}\n"
            )
    print(

...
from hyper_parameter import pilot_size, pilot_eps
import numpy as np
...
def plot_c_vs_likelihood(op, x, y_rejH0, y_acc):
    ...
    plt.title(
        f"HT acc & likelihood of rejecting H0 vs Pt (CSC, {H1_op}, eICU)",
        fontsize=18,
        fontweight="bold",
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"results_CNNH/CSC_{H1_op}/HTacc_"
        + Fname
        + "_"
        + H1_op
        + f"_query{str(seed)}_c{str(c)}_0801_30.png"
    )
...
if __name__ == "__main__":
    ...
    for Pt in pt_list:
        ...
        for range_ind in range(repeat_times):
            ...
            K_scaled = K_pt
            PT_start = time.time()
            PT_precision, PT_recall, _, _, PT_cost, PT_ans, _, _ = test_CSC_PT(
                Oracle_dist, Ranks, t=Dist_t, prob=Prob, pt=Pt, mode="exact", K=K_scaled
            )
            PT_end = time.time()
        ...
        backup_res = [
            ...
            round(sum(rt_k_CI_l) / len(rt_k_CI_l), 4),
            round(sum(rt_k_CI_h) / len(rt_k_CI_h), 4),
            0,
            # round(sum(rt_k_kl_divergence) / len(rt_k_kl_divergence), 4),
            round(sum(rt_cost) / len(rt_cost), 4),
        ]
        # print(backup_res)
        Path(f"./results_CNNH/CSC_{H1_op}/").mkdir(parents=True, exist_ok=True)
        with open(
            f"results_CNNH/CSC_{H1_op}/"
            + Fname
            + "_"
            + H1_op
            + f"_query{str(seed)}_c{str(c)}_0801_{repeat_times}.txt",
            "a",
        ) as file:
    ...
    # plt.legend(loc="best", prop={"size": 18, "weight": "bold"})
    # plt.savefig(
    #     f"results_CNNH/CSC_{H1_op}/agg_"
    #     + Fname
    #     + "_"
    #     + H1_op
    #     + f"_query{str(seed)}_c{str(c)}_0801_30.png"

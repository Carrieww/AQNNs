...
#         Path("./results_CNNH/PQA/").mkdir(parents=True, exist_ok=True)
#         with open(
#             "results_CNNH/PQA/"
#             + fname
#             + "_"
...
#             + "_"
#             + H1_op
#             + f"_{str(pt)[2:]}_{str(seed)}_0730.txt",
#             "a",
#         ) as file:
...
#     plt.title(
#         f"HT acc & likelihood of rejecting H0 vs Pt (PQA, {op}, eICU)",
#         fontsize=18,
#         fontweight="bold",
#     )
#     # plt.legend(loc="best", prop={"size": 18, "weight": "bold"})
#     plt.tight_layout()
#     plt.savefig(f"results_CNNH/PQA/HTacc_" + Fname + "_" + H1_op + f"_query{str(seed)}_c{str(c)}_0730.png")
...
if __name__ == "__main__":
    ...
    for Pt in pt_list:
        ...
        PT_start = time.time()
        _, _, k_star = test_PQA_PT(
            sync_Oracle_dist, phi, topk, t=Dist_t, prob=Prob, pt=Pt
        )

        if k_star == 0:
            # TODO: how?
            print("!!!!!!!!! OMG k_star is 0")
            pt_k_success.append(1)
            pt_k_precision.append(0)
            pt_k_recall.append(0)
            pt_response_time.append(round(time.time() - PT_start, 2))

            print(f"KL Divergence (PT): inf")
            # pt_k_kl_divergence.append(1000)
            # pt_k_agg.append(1000)
            # pt_k_acc.append(False)
            ...
            true_pos = len(np.intersect1d(ans_PT, true_ans_D))
            precision = true_pos / len(ans_PT)
            ...
            l_D_PT, agg_PT = agg_value(D_attr, ans_PT, attr_id, agg)
            ...
            print(
                f"The number of NN in D by PT is {len(ans_PT)} ({len(ans_PT)/Proxy_dist.shape[0]}%), the aggregated value is {agg_PT}"
            )
        ...
        # RT
        RT_start = time.time()
        _, _, k_star = test_PQA_RT(Proxy_dist, phi, topk, t=Dist_t, prob=Prob, rt=Rt)

        k_hat = int(k_star * (1 + 0 / 100))
        ans_RT = topk[:k_hat]

        true_pos = len(np.intersect1d(ans_RT, true_ans_D))
        precision = true_pos / len(ans_RT)
        if len(true_ans_D) == 0:
            recall = 0
        else:
            recall = true_pos / len(true_ans_D)
        ...
        # extract the target attribute in a list l_S
        l_D_RT, agg_RT = agg_value(D_attr, ans_RT, attr_id, agg)
        ...
        print(
            f"The number of NN in S is {len(ans_RT)} ({len(ans_RT)/Proxy_dist.shape[0]}%), the aggregated value is {agg_RT} (c = {c})"
        )
        # use l_S to perform t-test
        ...
        Path(f"./results_CNNH/PQA_{H1_op}/").mkdir(parents=True, exist_ok=True)
        with open(
            f"results_CNNH/PQA_{H1_op}/"
            + Fname
            + "_"
            + H1_op
            + f"_query{str(seed)}_c{str(c)}_0730.txt",
            "a",
        ) as file:
            results_str = "\t".join(map(str, backup_res)) + "\n"
            file.write(results_str)
    ...
    # plt.legend(loc="best", prop={"size": 18, "weight": "bold"})
    # plt.savefig(f"results_CNNH/PQA/agg_" + Fname + "_" + H1_op + f"_query{str(seed)}_c{str(c)}_0730.png")

    # plt.show()

    # fac_list = np.arange(58, 60.2, 0.5)
    # fac_list = [round(num, 4) for num in fac_list]
    # fac_list = [58]
    #
    # for c in fac_list:
    #     print(f"\nH1: {agg} {attr} {subject} {H1_op} {c}")
    #
    #     _, D_rejectH0 = HT_acc(l_D, c, H1_op, is_D=True)
    #
    #     for sample_size in sample_size_list:
    #         print(f">>>>> start sample size {sample_size}")
    #         exp_PQA_maximal_CR(
    #             D_attr,
    #             attr_id,
    #             sync_Oracle_dist,
